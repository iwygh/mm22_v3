...
!             & yhat,        zhat
    ...
    outfile = trim(adjustl(outfilestem)) // &
                ...
                & '_amax' // trim(adjustl(amaxstr)) // '_Nrep' // &
                & trim(adjustl(Nrepstr)) // '_set' // trim(adjustl(setstr)) // '.txt'
    ...
    ! open output file
    do irep = 1,Nrep
        write(*,*) amin,amax,irep,Nrep
        ! compute one rep of vm17method_2
        call vm17method2(amin,amax,Nsteps,ct_U,a_list,e_list,i_list,node_list,&
